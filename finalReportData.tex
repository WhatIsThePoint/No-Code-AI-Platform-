...
On the backend, every request is driven through the gateway on
\texttt{localhost:8000}. A call to \texttt{/auth/login} returns a JWT,
which is then presented as an \texttt{Authorization: Bearer} header on
the protected routes. The checks confirm the expected contract: the
health endpoint answers, login issues a token, and a protected route
such as \texttt{/datasets} returns \texttt{401} without a token and
\texttt{200} with a valid one, which is exactly the zero-trust boundary
the gateway is meant to enforce.
% [SCREENSHOT NEEDED: api_test_backend.png --- curl run through the gateway: health 200, login returning a token, /datasets 401 (no token) vs 200 (token), /billing/plans]

On the frontend, the \texttt{vitest} suite runs the component and store
tests headlessly and reports each file as it passes.
% [SCREENSHOT NEEDED: frontend_vitest.png --- npm test output, all suites passing]

% Once captured, reference the two screenshots here, e.g.\ ``as shown in Figure~\ref{fig:api_test_backend}'' and ``Figure~\ref{fig:frontend_vitest}''.

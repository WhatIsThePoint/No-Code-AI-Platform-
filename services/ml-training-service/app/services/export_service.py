...
- `serve.py`            — FastAPI server exposing POST /predict
...
# Note: this template is rendered with str.format, so every literal `{` or `}`
# in the Python source has to be doubled. The interpolated holes are
# {feature_columns_repr}, {algorithm}, {generated_at}, and {sample_payload}.
_TABULAR_SERVE_PY = '''\
"""
FastAPI inference server for the exported {algorithm} model.
...
Then POST a JSON body to /predict — see the auto-generated docs at /docs.
"""
...
_metadata: dict[str, Any] = json.loads((HERE / "metadata.json").read_text())
FEATURE_COLUMNS: list[str] = {feature_columns_repr}
TASK_TYPE: str = _metadata.get("task_type") or "classification"
TARGET_COLUMN: str | None = _metadata.get("target_column")

_model = joblib.load(HERE / "model.joblib")
_scaler = joblib.load(HERE / "scaler.joblib") if (HERE / "scaler.joblib").exists() else None
_imputer = joblib.load(HERE / "imputer.joblib") if (HERE / "imputer.joblib").exists() else None
...
class PredictRequest(BaseModel):
    features: list[dict[str, Any]] = Field(
        ...,
        description=(
            "List of feature rows. Each row must contain every column listed "
            "in the model's metadata.json under `feature_columns`."
        ),
    )
...
app = FastAPI(
    title="{algorithm} model server",
    description=(
        "Auto-generated inference endpoint for a No-Code AI Platform export. "
        "POST feature rows to /predict; see /docs for the schema."
    ),
    version="1.0.0",
)
...
def _prepare(rows: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    if _imputer is not None:
        df = pd.DataFrame(_imputer.transform(df), columns=FEATURE_COLUMNS)
    if _scaler is not None:
        df = pd.DataFrame(_scaler.transform(df), columns=FEATURE_COLUMNS)
    return df


@app.post("/predict", response_model=PredictResponse)
def predict(req: PredictRequest) -> PredictResponse:
    df = _prepare(req.features)
    raw = _model.predict(df)
    predictions = raw.tolist() if hasattr(raw, "tolist") else list(raw)

    probabilities: list[list[float]] | None = None
    if TASK_TYPE == "classification" and hasattr(_model, "predict_proba"):
        try:
...
# Minimal sample payload, useful for `curl -d @sample.json`:
SAMPLE_PAYLOAD = {sample_payload}


if __name__ == "__main__":
    # Allow `python serve.py` for quick local testing.
    import uvicorn
    uvicorn.run("serve:app", host="0.0.0.0", port=8000, reload=False)
'''


def _build_serve_py(feature_columns: list[str], algorithm: str, generated_at: str) -> str:
    """Render the FastAPI inference script for these feature columns."""
    cols = list(feature_columns or [])
    sample_row = {c: 0 for c in cols}
    sample_payload = repr({"features": [sample_row]})
    sample_features_inline = ", ".join(f'"{c}": 0' for c in cols[:8])
    serve_src = _TABULAR_SERVE_PY.format(
        feature_columns_repr=repr(cols),
        algorithm=algorithm,
        generated_at=generated_at,
        sample_payload=sample_payload,
    )
    # Validate the generated Python is syntactically correct before we ship it.
    # A bad template error here would only surface in the user's terminal.
    compile(serve_src, "serve.py", "exec")
    return serve_src, sample_features_inline
...
def build_tabular_zip(version_doc: dict) -> io.BytesIO:
    ...
    serve_src, sample_features_inline = _build_serve_py(
        feature_columns=feature_columns,
        algorithm=algorithm,
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        _add_text(
            ...
            _TABULAR_README.format(
                algorithm=algorithm,
                generated_at=metadata["exported_at"],
                sample_features=sample_features_inline or '"feature_1": 0',
            ),
        )
        _add_text(zf, "load_example.py", _TABULAR_LOAD_EXAMPLE)
        _add_text(zf, "serve.py", serve_src)
        _add_text(zf, "requirements.txt", _TABULAR_REQUIREMENTS)

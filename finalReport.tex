...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth, height=0.85\textheight, keepaspectratio]{diagrams/ch3_fig3}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\linewidth, height=0.95\textheight, keepaspectratio]{diagrams/ch3_fig_db_schema}
...
\midrule
Missing values     & column.missing\_pct $> 5\%$ & Impute with median (numeric) or mode (categorical); drop column if $> 50\%$. \\
High skew          & $|$skewness$| > 1$         & Log-transform the column before fitting a linear or distance-based model. \\
Outliers           & Tukey IQR count $> 1\%$ of rows & Cap to the 1st/99th percentile, or switch to a tree-based algorithm. \\
High cardinality   & unique\_pct $> 50\%$ on a categorical column & Target-encode instead of one-hot. \\
Target imbalance   & minority class $< 20\%$    & Prefer F1 over accuracy; consider class\_weight=balanced. \\
Multicollinearity  & $|$Pearson$| > 0.9$ between two numeric columns & Drop one of the pair before fitting a linear model. \\
...
\midrule
ML  & \texttt{DatasetNode}   & dataset\_id, target column, profile summary chip \\
    & \texttt{TrainNode}     & algorithm (one of fourteen), hyperparams JSON, task type \\
    & \texttt{EvaluateNode}  & metric keys to display, SHAP toggle, comparison link \\
\midrule
RAG & \texttt{DocumentNode}     & document\_id, file type, chunk count \\
    & \texttt{VectorStoreNode}  & pgvector index name, chunk size, overlap \\
    & \texttt{ChatNode}         & top-k, system prompt template, model name \\
\midrule
DL  & \texttt{ImageDatasetNode} & dataset\_id, class chips, thumbnail strip \\
    & \texttt{CNNArchNode}      & arch (LeNet / TinyResNet / MobileNetV3), input size, pretrained toggle \\
...
\midrule
Train / val / test split & $70\%$ / $15\%$ / $15\%$, stratified by target for classification, time-ordered for forecasting. Random seed pinned to $42$ for reproducibility. & --- \\
Missing-value imputation & Median for numeric columns, mode for categorical. Columns with $>50\%$ missing are dropped with a warning. & Tree-based algorithms tolerate missing values directly (XGBoost, LightGBM, CatBoost) --- imputation is still applied but configurable to off. \\
Feature scaling & StandardScaler on numeric columns (mean $0$, variance $1$). Fit on train only, applied to val/test. & Tree-based algorithms (scaling has no effect on splits). \\
Categorical encoding & One-hot for cardinality $\le 10$, target encoding (mean target value per category) for cardinality $> 10$. & CatBoost handles categoricals natively --- raw column passed through. \\
Outlier handling & Tukey IQR rule emitted as a warning; no automatic capping. User opts in via a checkbox on the Train node. & Robust algorithms (Random Forest, GBM). \\
Target transform & Identity. User can opt in to log-transform for skewed regression targets (toggle on the Train node). & Classification. \\
